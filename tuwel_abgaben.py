...
def course(cat, course_map):
    if cat in course_map:
        return course_map[cat]
    c = re.sub(r"-\d{4}S$", "", cat)
    c = re.sub(r"^\d+[\d.]*-?", "", c).strip("-")
...
def main():
    ...
    url = (
        f"https://tuwel.tuwien.ac.at/calendar/export_execute.php"
        f"?userid={userid}&authtoken={token}&preset_what=all&preset_time=monthnow"
    )
    ...
    course_map = {}
    for ev in events:
        cat = gf(ev, "CATEGORIES")
        s = gf(ev, "SUMMARY")
        m = re.match(r"[\d.]+\s+(.+?)\s*\([A-Z]{2}", s)
        if m and cat and cat not in course_map:
            course_map[cat] = m.group(1).strip().rstrip(",\\")
        elif "AlgoDat" in cat and cat not in course_map:
            m2 = re.match(r"[\d.]+\s+(.+?)\s*\(", s)
            if m2:
                course_map[cat] = m2.group(1).strip()
    ...
    if not results:
        ...
        for i, (dt, s, cat) in enumerate(results, 1):
            dm = dt.astimezone(MESZ)
            diff = dt - NOW
            print(f"  {i}. {clean(s)}")
            print(f"     Kurs:    {course(cat, course_map)}")
